    ...
    , ui(new Ui::MainWindow)
{
    ...
    //刷新列表
    connect(ui->btnFlushAddrList, &QPushButton::clicked, this, [=]() {
    ...
    connect(speedTimer, &QTimer::timeout, this, [=]() {
        speedLabel->setText(MSendFileThread::getSpeedSize());
    });
...
void MainWindow::toConnect(const QString &ip, const quint16 &port)
{
    ...
    //创建工作线程
    MConnect *work = new MConnect(ip, port);

    //连接结果
    connect(work, &MConnect::connectResult, this, [=](QString result) {
    ...
    //连接断开
    connect(work, &MConnect::overConnect, this, [=]() {
        work->deleteLater();
        initVariable();
    });

    //断开连接
    connect(this, &MainWindow::goToDisconnect, work, &MConnect::toDisconnect);

    //发送文件
    connect(this, &MainWindow::goToSendFile, work, &MConnect::toSendFile);

    //更新进度
    connect(work, &MConnect::updateSendProgress, ui->progressBar, &QProgressBar::setValue);
    connect(work, &MConnect::updateSendProgress, this, [=]() {
        if (!speedTimer->isActive())
            speedTimer->start(1000);
    });

    //全部文件发送完成
    connect(work, &MConnect::sendFinish, this, [=]() {
        isStartSend = false;
        speedTimer->stop();
